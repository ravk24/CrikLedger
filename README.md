...
| **Team viewer** | One shared **read-only** login per team, created by the superadmin (migration 49), who chooses both its user id and its password. **Up to 10 players at once** (migration 52, one seat per signed-in phone): an eleventh sign-in is refused with a message naming the superadmin who can free a seat, or asking someone signed in to log out in the WhatsApp group. A viewer's own logout frees only its seat. The viewer sees everything a member sees — balances, ledger, schedule, match sheets, statements, the team's tournaments — but nothing they can change: no credit/debit, no scheduling, no share images or export, no password change, no purchases. The superadmin sees every seat (device and sign-in time) on Manage admins and can **sign out one seat** or **sign out all** (same password still works), **reset the password**, or **remove** the login. |
...
| `/pool` | Team fund ledger (+ Credit/Debit and share-image buttons when logged in as admin) |
