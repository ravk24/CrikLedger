...
// The single scheduling flow: admins schedule free-form (date +
// opponent + ground name), and the pool debit for our ground share is
// written in the same transaction. The home-ground slot calendar that
// used to sit alongside this is gone — see
// CrikLedger-docs/dropped-home_match-feature.md.
async function ScheduleData() {
  ...
  if (!verdict.ok) {
    return (
      <AccessGate verdict={verdict} what="scheduling" next="/schedule/new" />
    );
  }
  ...
  return (
    <>
      {canSchedule && <ScheduleMatch captainName={captainName} />}
    ...
    <>
      <div>
        ...
        <p className="mt-0.5 text-xs text-text-muted">
          Put a new match on the calendar.
        </p>
      </div>
      <Suspense
        fallback={
