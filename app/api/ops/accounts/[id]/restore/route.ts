...
// Platform-level restore. This is the ONLY surface that writes
...
export async function POST(
  ...
  try {
    ...
    if (id === self.id) {
      throw new ApiError(409, "CANNOT_SUSPEND_SELF", "You cannot restore yourself");
    }

    const res = await pool.query(
      `UPDATE admins
          SET is_active = TRUE, session_epoch = session_epoch + 1
        WHERE id = $1
    ...
    return handleRouteError("[ops/restore]", error);
  }
}
